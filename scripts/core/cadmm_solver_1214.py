...
def _compute_local_cost(rid, q_candidate: LocalQ, 
                        env_state: EnvState, rl_action: RLAction, link_state: LinkState, aux, cadmm_params):
    """
    local cost:
    1) moving cost
    2) COI/explore cost
    ...
    if len(env_state.tasks) != 0:
        if role == 1:   
            role_task_weight = cadmm_params.role_relay_weight
        elif role == 0: 
            role_task_weight = cadmm_params.role_main_weight
        else:
            role_task_weight = cadmm_params.role_task_weight
        best_task_cost = None
        for task in env_state.tasks:
            task_id = task.id
            if task.status == "completed" or task.status == "failed": continue
            ...
            # 5) cost
            base = float(d)
            task_cost = role_task_weight * priority * urgency * base * 0.1
